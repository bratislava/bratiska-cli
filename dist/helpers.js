...
function assign_env_vars(options) {
  ...
  if (typeof sentry !== "undefined") {
    ...
    if (sentry === "***") {
      print_if_debug(options, `SENTRY_AUTH_TOKEN contains only stars in github actions, no value is passed.`);
    } else {
      print_if_debug(options, `SENTRY_AUTH_TOKEN(base64)=${Buffer.from(sentry).toString("base64")}`);
    }
