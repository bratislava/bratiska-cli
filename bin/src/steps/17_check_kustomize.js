...
function check_kustomize(options) {
  ...
  if (!fs_1.default.existsSync(manifest_path)) {
    throw new Error(
      `We had an error creating kustomize manifest. No kustomize file was found!`,
    );
  }
  const stats = fs_1.default.statSync(manifest_path);
  if (stats.size < 10) {
    throw new Error(
      `We had an error creating kustomize manifest. Kustomize file is empty! Check issue logs on kustomize build.`,
    );
  }
  helpers.ok();
}
